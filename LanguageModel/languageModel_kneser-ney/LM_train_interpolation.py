...
"""
    This file achieves to train the language model using the interpolation Kneser Ney. It can divide the ngrams, calculate the parameter D, calculate
...
for key in d3:
    ...
    keyb = str(key.split(' ')[2] + ' ' + key.split(' ')[1] + ' ' + key.split(' ')[0])
    # print(keyb)
    trie3b.insert(keyb)  # 二元逆序树
    keym = str(key.split(' ')[1] + ' ' + key.split(' ')[0] + ' ' + key.split(' ')[2])
    trie3m.insert(keym)
...
def pwi_1wi(wi_1wi):
    ...
    xwi_1wi_b = len(trie2b.get_start(str(wi_1wi_b + ' ')))  # *wi出现次数
    wi_1wi_bx = len(trie2.get_start(str(wi_1wi_b + ' ')))  # wi*出现次数

    p2 = (max((d2[wi_1wi] - db2), 0.0)) / (d1[wi_1wi_a]) + pow(10, grambp_1[wi_1wi_a]) * (xwi_1wi_b * 1.0 / ngram2)
...
def pwi_2wi_1wi(wi_2wi_1wi):
    ...
    wi_2wi_1wi_bb = str(wi_2wi_1wi.split(' ')[2] + ' ' + wi_2wi_1wi.split(' ')[1])
    wi_2wi_1wi_bm = str(wi_2wi_1wi.split(' ')[1])

    xwi_2wi_1wi_b = len(trie3b.get_start(str(wi_2wi_1wi_bb + ' ')))  # *wi-1wi出现次数
    wi_2wi_1wi_xbx = len(keym.get_start(str(wi_2wi_1wi_bm + ' ')))  # *wi-1*出现次数

    p3 = max((d3[wi_2wi_1wi] - db3), 0.0) / (d2[wi_2wi_1wi_a]) + pow(10, grambp_2[wi_2wi_1wi_a]) * (xwi_2wi_1wi_b * 1.0 / wi_2wi_1wi_xbx)
...
def arpa(address):
    ...
    for key in dicts:
        ...
        if i % 10000 == 0:
            print('1-gram: ' + str(i))
        if key == '</s>':
            gramp_1[key] = pwi(key)[0]
            arpaobject.write(str(gramp_1[key]) + '\t' + str(key))
            arpaobject.write('\n')
            ...
            gramp_1[key] = pwi(key)[0]
            grambp_1[key] = pwi(key)[1]
            arpaobject.write(str(gramp_1[key]) + '\t' + str(key) + '\t' + str(grambp_1[key]))
            arpaobject.write('\n')
    ...
    for key in d2:
        ...
        if key[(len(key) - len('</s>')):len(key)] == '</s>':
            gramp_2[key] = pwi_1wi(key)[0]
            arpaobject.write(str(gramp_2[key]) + '\t' + str(key))
            ...
            gramp_2[key] = pwi_1wi(key)[0]
            grambp_2[key] = pwi_1wi(key)[1]
            arpaobject.write(str(gramp_2[key]) + '\t' + str(key) + '\t' + str(grambp_2[key]))
